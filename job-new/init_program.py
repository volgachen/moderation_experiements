...
async def inference_function(
    ...
    quick_overview: string
        a string recorded that how the program processdd this message, that can be used to further analysis the reason why the inference function works well or not
    ...
    while True:
        try:
            ...
            quick_overview = f"LLM 的原始输出文本为：\n{raw_res}\n\n该输出在解析为DataFrame格式后，预测结果为：{'违规 (1)' if predicted else '未违规 (0)'}，原因为：{reason}"

            return batch_idx, merged, quick_overview

        except Exception as e:
            times_try += 1
            print(f"Batch {batch_idx + 1} failed, attempt {times_try}/5: {e}")
            if times_try >= 5:
                quick_overview = f"inference_function推理出错（已重试5次），报错信息如下：\n{e}"
                return batch_idx, None, quick_overview