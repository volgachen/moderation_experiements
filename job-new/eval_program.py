...
async def process(conf: Config, validate: bool):
    ...
    if validate:
        ...
        # 反馈样本数量信息
        feedback = await get_reflection(res_df, all_quick_overviews)
        return feedback
...
#     config = {"sample_nums": 20, "test": False, "api_key": "sk-3qUsDRiPEPyOsQbnD_C0lw", "validate_data_path": "http://cloud.staging.p1.cn/v2/ai-raw/153ecab0-213a-4d6b-9462-a469fe7c9697.parquet"}
#     program = open("init_program.py").read()
#     ans = asyncio.run(evaluate(program, config))
#     print(ans)
