...
async def inference_function(
    ...
    quick_overview: string
        a short setence describing how and why the inference_function get such a result
    ...
    while True:
        try:
            ...
            return batch_idx, merged, f"LLM经由prompt判断输入消息，并解析输出得到 {predicted}"

        except Exception as e:
            times_try += 1
            print(f"Batch {batch_idx + 1} failed, attempt {times_try}/5: {e}")
            if times_try >= 5:
                return batch_idx, None, "LLM 输出及解析逻辑失败"