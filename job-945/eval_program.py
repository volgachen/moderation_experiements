...
async def get_reflection(df_input, quick_overview):
    # withe the provided 
    system_message = """你是一名程序分析师，正在分析一个用户聊天行为审核程序的好坏，程序的目标是与人类审核员的审核结果高度对齐，从而能代替人类审核员直接开展审核工作。。我将会首先告诉你当前的用户聊天行为审核程序 <current_program> ，并给你提供一个具体的待审核消息 <input_example> ，当前程序的运行结果 <program_outputs> ，以及人类给予这条消息的审核结果 <human_ground_truth> 。请你仔细分析当前程序进行判断得到 <program_outputs> 时的分析与审核意图，理解人类审查员为何给出如此 <human_ground_truth> 及其背后的审核标准。如两者不一致，给出如何修改审查程序逻辑的具体建议，使其能与人类审核员的标准更好地对齐。

在牵手（该聊天平台的名称）的标注人员的培训手册上，标注了以下这些违规类型，可能能帮助你更好地分析问题：
...
请在充分分析和思考后，在 <reflection></reflection> 标签内阐述你的关键分析逻辑及结论。注意在标签内的总结应尽量逻辑完整的同时做到简洁干练，不泛泛而谈。

示例reflection：
当前程序输出判定该消息“毛病”属于轻度调侃或不满表达，不构成严重人身攻击，因而判定为未违规（0）。而人类审核员给出“违规 (1)”的判定，主要原因在于：人类审核员更严格地看待“毛病”作为对对话方的直接负面评价，视其为带有贬低性质的攻击，尤其是在对话中female多次强调“你删了，不好吧”后用“毛病”收尾，语气中带有明显责备和否定，体现了情绪上的否定态度，易被视作为人身攻击。
因此，程序应首先判断上下文中male和female的意图和攻击性，对攻击性强的一方，强化对直接负面词汇如“毛病”这类容易引发争议的词语的严格审核，尤其对于针对对话方的负面评价即使语气轻微也倾向判定为违规。
"""

    user_template = """# 当前的用户聊天行为审核程序
<current_program>
{current_program}
</current_program>
...
"""
    user_message = user_template.format(
        current_program = RAW_PROGRAM,
        message = df_input['comment'].iloc[0],
        context="\n".join(reversed(df_input['content'].iloc[0].splitlines())),
        program_outputs=quick_overview,
        human_ground_truth="违规 (1)" if df_input['label'].iloc[0] else "未违规 (0)",
    )
...
def format_df(df: pd.DataFrame, all_quick_overviews, all_reflections, threshold: float=1.0):
    out_FP = []
    out_FN = []
    out_TN = []
    out_TP = []
...
# {'\n'.join(out_TP)}
#     '''
    sampled_FP = random.sample(out_FP, min(len(out_FP), 10))
    if len(out_FP) > 10:
        sampled_FP.append('其余的省略。。。')
    sampled_FN = random.sample(out_FN, min(len(out_FN), 10))
    if len(out_FN) > 10:
...
async def inference_function_wrap(batch_idx, df_input, is_train):
    _, df_output, quick_overview = await inference_function(batch_idx, df_input)
    if is_train:
        reflection = await get_reflection(df_input, quick_overview)
    else:
        reflection = None
    return batch_idx, df_output, quick_overview, reflection
...
async def process(conf: Config, validate: bool):
    ...
    tasks = [
        asyncio.create_task(inference_function_wrap(i, b, is_train=not (conf.test or validate)))
        for i, b in enumerate(batches)
    ]
    all_results: List[pd.DataFrame] = [None] * len(batches)
    all_quick_overviews: List = [None] * len(batches)
    all_reflections: List = [None] * len(batches)
    ...
    for fut in asyncio.as_completed(tasks):
        idx, df_chunk, quick_overview, reflection = await fut
        finished += 1

        all_results[idx] = df_chunk
        all_quick_overviews[idx] = quick_overview
        all_reflections[idx] = reflection
    ...
    if validate:
        ...
        feedback += format_df(res_df, all_quick_overviews, all_reflections)
        return feedback
