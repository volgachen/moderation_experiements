...
class HotpotQA_HoverMultiHop(dspy.Module):
    ...
    def forward(self, claim):
        ...
        answer = dspy.Prediction(retrieved_docs=hop1_docs + hop2_docs + hop3_docs)
...
async def get_hotpotqa_response_per_sys_priompt(user_prompt, answer, sys_prompt, top_k_search, lm):
    ...
    pred_answer = [c.split(" | ")[0] for c in res['answer'].retrieved_docs]
    perform = discrete_retrieval_eval(answer, pred_answer)
    res.update({"acc": perform})
    res.update({"pred_answer": pred_answer})
    return res
...
async def get_response_major_voting(i, sampled_user_prompts, sampled_answers, system_prompts, top_k_search, lm, majority_answers, group_answers):
    ...
    if len(system_prompts) == 1:
        group_answers[i] = res_s[0]['pred_answer']
    else:
        res_answers = [res_s[i]['pred_answer'] for i in range(len(system_prompts))]
...
async def release_one_group_for_questions(
    ...
    if n_question_sample == "full":
        ...
        for j in range(len(system_prompts)):
            system_prompt_answer = [majority_answers[i][j]['pred_answer'] for i in range(len(sampled_answers))]
...
async def evaluate(program, config=None) -> EvalResult:
    ...
    eval_results.feedback = feedbacks[0] + "==^&*(split-part)==" + json.dumps(all_final_answers, ensure_ascii=False)
